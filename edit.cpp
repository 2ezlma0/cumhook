...
void Edit::draw( ) {
	...
	// draw label.
	render::menu.string( p.x + EDIT_X_OFFSET, p.y - 2, { 205, 205, 205, 255 }, m_label );

	// box outline.
	render::rect( p.x + EDIT_X_OFFSET, p.y + 15, m_w - EDIT_X_OFFSET, EDIT_BOX_HEIGHT, { 250, 250, 250, 255 } );

	// inner blob
	// todo; gradient?
	render::rect_filled( p.x + EDIT_X_OFFSET + 1, p.y + 15 + 1, m_w - EDIT_X_OFFSET - 2, EDIT_BOX_HEIGHT - 2, { 241, 241, 241, 255 } );
	...
	if( m_typing && ( g_winapi.GetTickCount( ) / 500 ) % 2 )
		render::rect_filled( p.x + EDIT_X_OFFSET + EDIT_ITEM_X_OFFSET + size.m_width + 1, p.y + 15 + 13, 6, 2, { 252, 252, 252, 255 } );
}
