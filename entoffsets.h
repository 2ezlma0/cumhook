...
public:
    ...
    XoredNetvar< GET_XOR_KEYUI32, __COUNTER__ > m_bWaitForNoAttack;
...
#pragma optimize( "", off ) // типа выделяешь код и он не опимизируется по идее
    ...
    void init( ) {
        ...
        INIT_XOREDNETVAR( m_bWaitForNoAttack,                   HASH( "DT_CSPlayer" ),             HASH( "m_bWaitForNoAttack" ) );
        ...
        INIT_XOREDNETVAR( m_flGroundAccelLinearFracLastTime,    HASH( "DT_CSPlayer" ),         HASH( "m_flGroundAccelLinearFracLastTime" ) );
        ...
        INIT_XOREDNETVAR_SPECIFIC( m_pCurrentCommand,            g_netvars.get( HASH( "DT_BasePlayer" ), HASH( "m_hConstraintEntity" ) ) - 0xC );
